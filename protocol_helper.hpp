...
namespace protocol_helper
{
    ...
    /// Returns the mask necessary to select the number of bits from the right
    /**
     *  @tparam Bits the number of bits from the right of an unsigned char
     *  to select
     */
    template<size_t Bits>
    struct rbit_mask
    {
	enum : unsigned char { value = (1 << (Bits - 1)) + rbit_mask<Bits - 1>::value };
    };

    /// Base implementation of rbit_mask
    template<>
    struct rbit_mask<0>
    {
    ...
    struct lbit_mask
    {
	enum : unsigned char { value = (1 << (8 - Bits)) + lbit_mask<Bits - 1>::value };
    };
    ...
    {
	static const T value(unsigned char const * const buf) {
	    return buf[0] & lbit_mask<Field_Bits>::value;
	}
    };
    ...
    {
	static const T value(unsigned char const * const buf) {
	    const size_t bits = 8 - (Field_Offset % 8);
	    return
		((buf[0] & rbit_mask<bits>::value) << 8) +
